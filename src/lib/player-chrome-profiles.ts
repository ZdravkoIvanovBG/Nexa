...
export function importProfileJson(
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return { ok: false, error: "File is not a Nexa layout profile." };
  }
  const obj = parsed as Record<string, unknown>;
  if (obj.harborProfileVersion !== undefined && obj.harborProfileVersion !== 1) {
    return {
      ok: false,
      error: `Unsupported profile version (${String(obj.harborProfileVersion)}). Update Nexa or use an older profile.`,
    };
  }
