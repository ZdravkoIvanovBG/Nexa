...
export const ENGINE_WARMING_MESSAGE =
  "Nexa's peer-to-peer engine is warming up. This clears on its own in a few seconds, then Play works.";
...
export function humanError(code: string): string {
  switch (code) {
    ...
    case "external-url-only":
      return "This source only opens in an external browser, not in Nexa's player.";
    case "youtube-only":
